...
const fetchImage = () => {
  fetch(`https://api.unsplash.com/search/photos/?client_id=cS0krMKce7U53S2biX1d-5x6oGAHrOh_YJecqqnBAmo&per_page=50&query=${value}`)
  .then(res =>res.json())
  .then(data => {
  ...
  return (
    <div className="App flex items-center justify-center w-full">
      ...
      <div className='flex w-full items-center justify-around text-purple-950 bg-purple-400  p-4 shadow-xl md:text-xl'>
        ...
        <input placeholder='search for any type of photos' type='text' value={value} onChange={(e) => setValue(e.target.value.toLowerCase())} className='p-2 rounded-xl w-72 md:w-96 lg:w-full lg:max-w-xl md:text-lg outline-none'onKeyDown={fetchImage}/>
